...
import { Send, Link as LinkIcon, Loader2, Image as ImageIcon } from "lucide-react";
import { io, Socket } from "socket.io-client";
import { SOCKET_URL } from "@/lib/constants";
import useSWR from "swr";
import { InteractiveIconWrapper, AnimatedEmptyState } from "@/components/Animations";
...
export function ChatClient({ spaceId, userId, userName }: { spaceId: string, userId: string, userName: string }) {
  const { data, isLoading } = useSWR(`/api/spaces/${spaceId}/messages?limit=100`, fetcher);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-16rem)] max-h-[800px] border border-white/5 bg-[#0A0503]/40 backdrop-blur-xl rounded-[2rem] overflow-hidden">
      {/* Header */}
      <div className="p-6 border-b border-white/5 bg-white/5">
        <h2 className="text-xl font-serif italic text-white flex items-center gap-2">
          Discussion 
          <span className="text-xs font-sans not-italic text-orange-400 bg-orange-500/10 px-2 py-0.5 rounded-full">Temps réel</span>
        </h2>
        <p className="text-xs text-[#A0A0A0] mt-1">Chat intégré pour collaborer directement dans l'espace.</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full opacity-50">
            ...
            <p className="text-sm font-bold mt-4 text-[#A0A0A0]">Aucun message.</p>
            <p className="text-xs text-[#666666]">Soyez le premier à lancer la discussion !</p>
          </div>
        ) : (
          messages.map((msg, i) => {
                ...
                >
                  ...
                  {msg.preview && Object.keys(msg.preview).length > 0 && (
                    ...
                    >
                      ...
                      <div className="p-3">
                        <p className="text-xs font-bold text-white line-clamp-1">{msg.preview.title}</p>
                        <p className="text-[10px] text-[#A0A0A0] line-clamp-2 mt-1">{msg.preview.description}</p>
                        <p className="text-[9px] text-orange-400 mt-2 uppercase tracking-widest flex items-center gap-1">
                          <LinkIcon className="w-3 h-3" /> {msg.preview.siteName || new URL(msg.preview.url || "https://swiftdrop.app").hostname}
                        </p>
                      </div>
                    </a>
                  )}
                  
                  <div className={`text-[9px] opacity-50 mt-2 flex items-center gap-1 ${isMe ? 'justify-end' : 'justify-start'}`}>
